...
export function InkBreakthroughModal({ result, onClose }: Props) {
  ...
  const cls = (base: string, stillCls?: string) =>
    stillClassName(base, stillCls, still);
  ...
  return createPortal(
    <div
      ...
      aria-modal="true"
      aria-label={
        ok ? `突破關口：${result.oldTierName}進至${bigText}` : "走火入魔"
      }
      onClick={onClose}
    >
      <div className={styles.grain} aria-hidden />
      <div
        className={`${styles.stage}${ok ? "" : ` ${cls(styles.shake, styles.shakeStill)}`}`}
      >
        ...
        <div className={styles.body}>
          ...
          <ul className={styles.deltas}>
            {ok ? (
              <>
                {typeof result.martialGain === "number" && (
                  <li>武學＋{result.martialGain}</li>
                )}
                <li>氣血上限、內力上限同步提升</li>
              </>
            ) : (
              <>
                {typeof result.hpLoss === "number" && (
                  <li>氣血－{result.hpLoss}</li>
                )}
                {typeof result.qiLoss === "number" && (
                  <li>內力－{result.qiLoss}</li>
                )}
